    ...
    apply = function(self, tag, context)
        if context.type == 'store_joker_create' and not tag.triggered then
            ...
            if #vtubers > 0 then
                ...
                local card = create_card('Joker', context.area, nil, nil, nil, nil, chosen.key, 'twitch_tag')
                create_shop_card_ui(card, 'Joker', context.area)
                card:set_edition('e_vtb_hyped', true)
                card.states.visible = false
                card:start_materialize()
                tag:yep('+Hyped Joker!', G.C.DARK_EDITION, function()
